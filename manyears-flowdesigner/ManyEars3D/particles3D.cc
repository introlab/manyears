...
float Particle3D::prob(float pos[3], float std_1)
{
#ifdef DISTANCE
   float xx[3], yy[3];
   float xdist, ydist;
   xdist = sqrt(.001f+sqr(x[0]) + sqr(x[1]) + sqr(x[2]));
   ydist = sqrt(.001f+sqr(pos[0]) + sqr(pos[1]) + sqr(pos[2]));
   float distdist = sqr(xdist - ydist)/(xdist*ydist);
   distdist = .2+.8*exp(-30.f*distdist);
   xdist = 1.f/xdist;
   ydist = 1.f/ydist;
   float dist = sqr(pos[0]*ydist - x[0]*xdist) + sqr(pos[1]*ydist - x[1]*xdist) + sqr(pos[2]*ydist - x[2]*xdist);
   dist = -std_1*std_1*dist;
   if (dist > -70)
      return 1e-30+distdist*.8f*exp(4.f*dist)+.18f*exp(.4f*dist)+.02f*exp(.1f*dist);
   else
      return 1e-30;
...
void Particle3D::updateWeight(float pos[3], float std_1)
{
...
Particle3DFilter::Particle3DFilter(float pos[3], float std, int N, float initial_prob, int count)
   : particles(N, pos)
   , weight(initial_prob)
   , srcExists(false)
   , age(0)
   ...
   , aligned(0)
{
   for (int i=0;i<particles.size();i++)
   {
      ...
      float x = ran();
      if (x<.2)
         particles[i].state = Particle3D::Stopped;
      else if (x<.5)
         particles[i].state = Particle3D::ConstVelocity;
      else
         particles[i].state = Particle3D::Excitation;
      if (particles[i].state != Particle3D::Stopped)
      {
...
Particle3DFilter::~Particle3DFilter()
{
   if (exists())
   {
...
void Particle3DFilter::predict()
{
   for (int i=0;i<particles.size();i++)
      particles[i].predict();
}

float Particle3DFilter::prob(float pos[3], float std)
{
   float weightSum = 0;
   float std_1 = 1./std;
   for (int i=0;i<particles.size();i++)
...
void Particle3DFilter::update(float pos[][3], float probs[], int N, float std[], int count)
{
   float sum_prob = 0;
   float std_1[N];
   for (int i=0;i<N;i++)
...
void Particle3DFilter::resample()
{
   ...
   if (effectiveN > .6)
      return;
   vector<Particle3D> tmp = particles;
...
void Particle3DFilter::getStats(float mean[3], float std[3])
{
   for (int j=0;j<3;j++)
   {
      mean[j] = delayedPos[j];
...
void Particle3DFilter::updateStats(bool aligned, int count)
{
   ...
   if (0) {
      for (int i=0;i<particles.size();i++)
      {
         cout << "AA" << (count-3)/4 << " " << ID << " " << particles[i].x[0] << " " << particles[i].x[1] << " " << particles[i].x[2] << endl;
      }
   }
}
...
Particle3DMixture::Particle3DMixture(int N, float std)
   : nextID(0)
   , nbParticle3Ds(N)
   , estimStd(std)
{
...
Particle3DMixture::~Particle3DMixture()
{
...
void Particle3DMixture::resample()
{
   for (int i=0;i<filters.size();i++)
      filters[i]->resample();
}
   
void Particle3DMixture::predict()
{
   for (int i=0;i<filters.size();i++)
      filters[i]->predict();
}
...
#define NEW_ALGO
void Particle3DMixture::update(float pos[][3], float probs[], int N, float std[], int count)
{
   ...
   for (int i=0;i<N;i++)
   {
      ...
      {
         filters.push_back(new Particle3DFilter (pos[i], estimStd, nbParticle3Ds, new_src_prob[i], count));
   ...
   {
      bool remove = false;
      if (/*filters[i]->prob_exist < .01 || */(filters[i]->aligned == 0 && filters[i]->age > 4) || (filters[i]->aligned == 1 && filters[i]->age > 6) || (filters[i]->aligned < 4 && filters[i]->age > 8) || (filters[i]->aligned < 8 && filters[i]->age > 12) || (filters[i]->age2 > 25))
