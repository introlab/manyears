...
void *savewav_rec_thread(void *data);


class SaveWav : public BufferedNode {
  
  friend void *savewav_rec_thread(void *data);
...
public:
  SaveWav(string nodeName, ParameterSet params)
    : BufferedNode(nodeName, params), m_saveToFile(false), m_splitStrength(0)
  {
    ...
    pthread_create(&m_thread, NULL, savewav_rec_thread, this);
  }
...
void *savewav_rec_thread(void *data)
{
	...
	SaveWav *node = reinterpret_cast<SaveWav*>(data);
